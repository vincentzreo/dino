...
pub(crate) fn get_files_with_exts(dir: &str, exts: &[&str]) -> Result<BTreeSet<PathBuf>> {
    ...
    for ext in exts {
        ...
        let path = glob(&pattern)?.collect::<Result<BTreeSet<PathBuf>, GlobError>>()?;
        files.extend(path);
    }
    Ok(files)
}
...
mod tests {
    ...
    #[test]
    fn get_files_with_exts_should_work() -> anyhow::Result<()> {
        ...
        assert_eq!(
            ...
            vec![
                ...
                PathBuf::from("fixtures/prj/test2/d.json"),
            ]
        );
        Ok(())
    }
    ...
    #[test]
    fn calc_hash_for_files_should_work() -> anyhow::Result<()> {
